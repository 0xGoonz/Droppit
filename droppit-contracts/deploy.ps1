...
function Resolve-ForgePath() {
    $configuredPath = Resolve-EnvValue -Names @("FORGE_BIN") -Required $false
    if (-not [string]::IsNullOrWhiteSpace($configuredPath)) {
        if (!(Test-Path $configuredPath)) {
            throw "forge not found at FORGE_BIN path: $configuredPath"
        }

        return $configuredPath
    }

    if ($null -eq (Get-Command forge -ErrorAction SilentlyContinue)) {
        throw "forge not found in PATH. Install Foundry or set FORGE_BIN to the forge executable path."
    }
