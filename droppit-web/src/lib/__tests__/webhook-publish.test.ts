...
describe("Webhook Integration", () => {
    ...
    it("creates a draft and publishes one deploy reply for a valid mention", async () => {
        ...
        expect(mockState.ensureAgentPostOutboxRecord).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            ...
            requestPayload: expect.objectContaining({
                text: expect.stringMatching(/^@testuser Draft ready: Midnight Run \| 100 editions \| Free mint\. Review & deploy it in Droppit below\.$/),
                parent: "0xcast123",
        ...
        expect(json.frame.image).toBe("http://localhost:3000/api/og/draft/draft-1?variant=miniapp");
        expect(json.frame.buttons[0].label).toBe("Review & Deploy");
    ...
    it("publishes a remediation reply and skips draft creation for invalid intents", async () => {
        ...
        const remediationText = mockState.ensureAgentPostOutboxRecord.mock.calls[0][1].requestPayload.text;
        expect(remediationText).not.toContain("@testuser");
    ...
    it("keeps a text-only draft path when media validation fails", async () => {
        ...
        expect(mockState.ensureAgentPostOutboxRecord).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            requestPayload: expect.objectContaining({
                text: expect.stringContaining("Upload a high-res artwork in Droppit before deploying."),
            }),
        }));
        expect(json.success).toBe(true);
        expect(json.frame.buttons[0].label).toBe("Upload High-Res");
    });

    it("falls back to the existing success text when the author username is blank", async () => {
        const res = await POST(createWebhookReq({
            ...VALID_CAST_BODY,
            data: {
                ...VALID_CAST_BODY.data,
                author: {
                    ...VALID_CAST_BODY.data.author,
                    username: "   ",
                },
            },
        }));
        ...
        expect(res.status).toBe(200);
        expect(json.success).toBe(true);
        const replyText = mockState.ensureAgentPostOutboxRecord.mock.calls[0][1].requestPayload.text;
        expect(replyText.startsWith("@")).toBe(false);
        expect(replyText).toContain("Draft ready: Midnight Run | 100 editions | Free mint.");
    });
    ...
    it("keeps success replies within the cast limit when the mention and title are long", async () => {
        mockState.parseDeployIntent.mockResolvedValue({
            success: true,
            title: "Midnight Run ".repeat(30).trim(),
            editionSize: 100,
            mintPrice: "0",
            assetUri: undefined,
        });
        ...
        expect(json.success).toBe(true);
        const replyText = mockState.ensureAgentPostOutboxRecord.mock.calls[0][1].requestPayload.text;
        expect(replyText.startsWith("@testuser Draft ready:")).toBe(true);
