...
export function buildDeployReplyText(params: {
    ...
    const authorUsername = typeof params.authorUsername === "string"
        ? params.authorUsername.trim().replace(/^@+/, "")
        : "";
    const mentionPrefix = authorUsername ? `@${authorUsername} ` : "";
    const replyBody = params.hasReusableMedia
        ? `Draft ready: ${title} | ${params.editionSize} editions | ${priceLabel}. Review & deploy it in Droppit below.`
        : `Draft saved: ${title} | ${params.editionSize} editions | ${priceLabel}. Upload a high-res artwork in Droppit before deploying.`;

    return normalizeReplyText(
        `${mentionPrefix}${replyBody}`,
