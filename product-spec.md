...
**Agentic distribution layer (MVP+):** A proactive Droppit persona on Farcaster supports natural-language drop creation and deploy confirmation Mini Apps today; broader milestone/curation posting remains expanded scope powered by backend analytics signals.
...
2. **Link identity (optional)**
   - **Link Farcaster handle (optional)**
     ...
     - App issues a one-time **link nonce** (expires in 10 minutes) and asks the creator wallet to sign:
       ...
       Chain: <active Base network> (chainId <8453|84532>)
       Nonce: <random>
       Issued At: <ISO8601>
       Expires At: <ISO8601>
       ```
...
- **Immutability strength (MVP):** Base guarantee is **policy + backend-enforced immutability** (frozen server record; no post-publish update path).
- **Current implementation (locked-content drops):** publish/deploy flow also records a salted `lockedMessageCommitment` (`bytes32`) during `initialize(...)` and emits an event (e.g., `LockedMessageCommitted(commitment)`). For drops without locked content, this value may remain zero/unset.
...
**Locked message commitment (current support for locked-content drops):**

- When locked content is provided, pass `lockedMessageCommitment: bytes32` at initialize-time and emit `LockedMessageCommitted(bytes32 commitment)`.
- Commitment represents a salted hash (or ciphertext hash) of the locked message. Plaintext is never stored onchain.
- For drops without locked content, the commitment value may remain zero/unset.
...
- Base support; development/test flows may use Base Sepolia depending on environment/configuration
- Wrong-network prompt to switch to the active configured Base network
...
- **RPC Provider:** **Alchemy** for Base network access (mainnet and/or Base Sepolia, configured via `.env` such as `NEXT_PUBLIC_ALCHEMY_API_KEY`) to ensure high limits for OnchainKit/viem.
...
* **Frames (MVP must-have):**
  ...
  - **Chain (MVP):** active configured Base network, returned as a **CAIP-2** chainId string (for example `eip155:8453` on Base mainnet or `eip155:84532` on Base Sepolia).

  - **Minimum response shape (MVP):**

    ```json
    {
      "chainId": "<active Base CAIP-2 chainId>",
...
- **Immutability strength (MVP):** frozen **offchain** (server-enforced), with an onchain commitment currently recorded for locked-content drops.
- If `lockedMessageCommitment` exists, show a commitment indicator (e.g., "Commitment verified") and reference the commitment field/event for verifiability.
...
- Validate file type/size
  ...
  - Enforce max upload size (e.g., 20MB) and sniff MIME/type **client-side** before requesting a backend-issued temporary Pinata upload credential (current flow caps it at `maxUses: 2`) to upload image + metadata directly to IPFS, bypassing Vercel payload limits.
...
Track (MVP target surface; current implementation is partial):
...
- **Autonomous persona impact (expanded scope):** milestone quote-cast impressions/clickthrough and daily curation post engagement

### 11.1 Autonomous Farcaster Persona (MVP+)

- Droppit currently supports mention-triggered Farcaster replies for remediation and deploy-assist flows, not only passive command parsing.
- **Persona Vibe (product direction):** **"Hybrid: Sassy Hype-man"**. High energy, extremely supportive (LFG, fire), with a witty edge that keeps Farcaster engagement fun and meme-able.
- Richer persona modules / prompt configuration (e.g., `soul.md`) remain expanded scope rather than an MVP assumption.
- **Current implemented baseline:** failed parses get remediation replies; successful parses get deploy-assist replies with canonical Droppit links/embeds.
- **Expanded scope:** automated milestone quote-casts and daily curation posting remain future extensions powered by analytics signals.
...
Note: See [roadmap.md](./roadmap.md) for current execution priorities and shipped-status tracking. This spec remains the source of truth for product scope and requirements.
...
- Reveal payload: *Never Gonna Give You Up* lyrics + `... guess who got rickrolled`.
...
**Tooling:** Supabase-backed server event log + light client tracking, with `/api/attribution/view` and `/api/attribution/mint` as the current baseline endpoints.

**Client-side events (target catalogue; currently partially wired):**
...
**Server-side event log (current baseline):**

- Table: `analytics_events`
- Currently used for referral attribution, creator stats, and incremental funnel analysis
- Live rows currently center on `page_view`, `mint_success`, and selected frame interaction events; broader catalogue coverage is still being filled in
- Reference row shape: `{ event, dropId, contractAddress, ref, refType, selfRef, wallet, timestamp, sessionId }`
**Referral attribution events** use the existing `/api/attribution/view` and `/api/attribution/mint` endpoints per Section 8. Broader client-side event coverage can expand incrementally around that baseline.
